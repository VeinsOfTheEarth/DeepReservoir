...
class NavajoReservoirEnv(Env):
    ...
    def __init__(
        ...
        max_total_release_cfs: float = 5000.0,   # <-- IMPORTANT: total cap like Colab
        is_eval: bool = False,
    ):
        super().__init__()
        ...
        self.episode_length = int(episode_length) if episode_length is not None else self.n_steps
        ...
        # ---- Observation schema (fixed order, Colab-aligned) ----
        # Colab obs was: storage, evaporation, inflow, doy
        required_cols = ["inflow_cfs", "evap_af"]  # storage is simulated; doy is computed
        for c in required_cols:
            if c not in self.data_raw.columns:
                raise KeyError(f"data_raw is missing required column '{c}'")
        # For normalized forcings (inflow/evap), we expect same names in data_norm
        for c in ["inflow_cfs", "evap_af"]:
        ...
        # Keep schema stable across train/eval
    ...
    def step(self, action):
        ...
        # First map to [0, max_total] each (like Colab's per-action scaling),
        # then we will cap total to max_total_release_cfs.
        rel0_cfs = self.min_release_cfs + frac_0 * (self.max_total_release_cfs - self.min_release_cfs)
        rel1_cfs = self.min_release_cfs + frac_1 * (self.max_total_release_cfs - self.min_release_cfs)
        ...
        if requested_total_cfs > self.max_total_release_cfs:
            ...
            cap_penalty = (requested_total_cfs - total_cfs) / (self.max_total_release_cfs + 1e-9)
        else:
        ...
        if total_cfs > max_total_cfs_phys:
            ...
            rel1_cfs *= scale
            total_cfs = rel0_cfs + rel1_cfs
            phys_penalty = (max_total_cfs_phys - total_cfs) / (self.max_total_release_cfs + 1e-9)
        ...
        # Optional Farmington proxy (if those columns exist in raw)
        animas_cfs = float(row_raw["animas_farmington_q_cfs"]) if "animas_farmington_q_cfs" in row_raw.index else 0.0
        sj_at_farm_cfs = animas_cfs + total_cfs  # total release affects downstream
        self.sj_at_farmington_history.append(sj_at_farm_cfs)
        sj_at_farm_lag2_cfs = self.sj_at_farmington_history[-3] if len(self.sj_at_farmington_history) >= 3 else None

        # Elevation + hydropower (Colab intent: hydropower from component #2)
        new_elev_ft = float(self.capacity_to_elev(new_storage_af))
        hydropower_mwh = navajo_power_generation_model(
            cfs_values=float(rel1_cfs),     # <-- component #2 drives power, like your Colab Q2
        ...
        info = {
            ...
            # Keep existing keys for repo compatibility, even if you interpret them differently
            "sanjuan_release_cfs": float(rel0_cfs),   # component #1
            "niip_release_cfs": float(rel1_cfs),      # component #2 (hydropower in Colab)
